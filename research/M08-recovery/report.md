...
状态：已实现/待完整集成。检索日期：2026-09-07；2026-09-08 已实现受限文本、Hex/Base64、gzip/zlib 恢复及来源链。
...
本地验证边界：合成真值覆盖嵌套 Base64→gzip、UTF-16、错误编码、截断压缩、安全上限、源哈希和无密钥 TLS 跳过。真实样本编码、压缩封装及是否存在完整对象或会话密钥仍待核实。Base64/Hex 是编码，不是加密；没有适用密钥不承诺恢复 TLS/AES 明文。随机密文里偶然出现可打印字符串，只能报告为片段候选。
