...
已按固定上游提交获取两个原始 `.dat`，保存于 `data/external/raw/`，来源、许可、SHA-256、敏感性与格式真值记录在 `data/external/manifest.json`：

1. Audiobeat OctaPro 8.2 的 `dsp_m2.dat`：真实设备配置经厂商应用导出的 2387 字节 US002 二进制预设，上游有格式文档、离线解析器和测试。适合 M1～M6/M8，但没有时间戳或方向，不能用于 M9～M11。
