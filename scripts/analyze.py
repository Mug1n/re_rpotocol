...
try:
    ...
    # Older compatible B-line checkouts expose direct recovery only.  Profiles
    # that do not select M08 (including unknown-private) must remain usable.
    from experiments.M08.run import analyze_recovery

    def analyze_payload_source(*args: Any, **kwargs: Any) -> dict[str, Any]:
        raise RuntimeError("this checkout does not provide payload-source recovery; remove M08 or update M08")
...
try:
    ...
    def extract_payload_sources(*args: Any, **kwargs: Any) -> dict[str, Any]:
        raise RuntimeError("this checkout does not provide HTTP payload-source extraction; remove PAYLOAD_SOURCES or update B artifacts")
